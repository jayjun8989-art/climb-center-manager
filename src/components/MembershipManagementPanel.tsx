...
export function MembershipManagementPanel({
  ...
  const [pauseTarget, setPauseTarget] = useState<MemberListItem | null>(null);
  ...
  async function handlePauseConfirm(
    ...
    try {
      ...
      onUpdated(updated);
      onNotify(`회원권을 ${pauseDays}일 일시정지했습니다.`);
    } catch (error) {
      onNotify(formatAppError(error));
    }
    setPauseTarget(null);
  ...
  return (
    <>
      ...
      {pauseTarget && (
        <PauseModal
          memberName={pauseTarget.name}
          onClose={() => setPauseTarget(null)}
          onConfirm={(pauseDays, reason, startDate, endDate) =>
            void handlePauseConfirm(pauseTarget, pauseDays, reason, startDate, endDate)
          }
        />
      )}
    </>
