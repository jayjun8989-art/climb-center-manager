...
export async function fetchRoster(center: Center | "ALL"): Promise<RosterRow[]> {
  ...
  if (center !== "ALL") {
    builder = builder.eq("center_code", center);
  }
...
export function statusCategory(row: RosterRow, today: string): "유효회원" | "만료예정" | "만료소진" | "정지회원" | "회원권없음" {
  ...
  if (
    (row.remaining_sessions !== null && row.remaining_sessions <= 0) &&
...
export async function fetchAuditLogs(filters: {
  ...
  if (filters.action && filters.action !== "ALL") {
    if (filters.action === "delete") {
      builder = builder.in("action", ["delete", "soft_delete"]);
    } else {
...
export async function fetchTodayAttendance(center: Center | "ALL"): Promise<TodayAttendanceRow[]> {
  ...
  let builder = supabase
    ...
    .select(
      "id, checkin_at, deducted_count, member_id, center_id, centers(code), members(name, member_type)"
    )
